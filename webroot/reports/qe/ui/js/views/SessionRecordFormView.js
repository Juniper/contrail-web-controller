...
define([
    ...
    var SessionRecordQueryView = QueryFormView.extend({
        ...
        renderSessionAnalyzer: function(selectedSessionRecord) {
            var self = this,
                viewConfig = self.attributes.viewConfig,
                ...
                queryFormId = cowc.QE_HASH_ELEMENT_PREFIX + cowc.SR_QUERY_PREFIX + cowc.QE_FORM_SUFFIX,
                queryResultId = cowc.QE_HASH_ELEMENT_PREFIX + cowc.SR_QUERY_PREFIX + cowc.QE_RESULTS_SUFFIX,
                queryResultTabsId = cowl.QE_SESSION_RECORD_TAB_ID,
                queryResultTabsView = contrail.checkIfExist(childViewMap[queryResultTabsId]) ? childViewMap[queryResultTabsId] : null;

            selectedSessionRecord = contrail.checkIfExist(selectedSessionRecord) ? selectedSessionRecord : viewConfig.selectedSessionRecord;
            if (widgetConfig !== null) {
                $(queryFormId).parents(".widget-box").data("widget-action").collapse();
            }
            ...
            if (!contrail.checkIfExist(queryResultTabsView)) {
                self.renderView4Config($(queryResultId), null,
                    getSessionAnalyzerTabsViewConfig(queryResultTabsId, queryFormAttributes, selectedSessionRecord),
                    null, null, modelMap, null);
            } else {
                queryResultTabsView.renderNewTab(queryResultTabsId,
                    getSessionAnalyzerTabViewConfig(queryFormAttributes, selectedSessionRecord), true, modelMap, null);
            }
        },
    ...
    function getSessionAnalyzerTabsViewConfig(queryResultTabsId, queryFormAttributes, selectedSessionRecord) {
        return {
            elementId: queryResultTabsId,
            view: "TabsView",
            viewConfig: {
                theme: cowc.TAB_THEME_WIDGET_CLASSIC,
                tabs: getSessionAnalyzerTabViewConfig(queryFormAttributes, selectedSessionRecord)
            }
        };
    }

    function getSessionAnalyzerTabViewConfig(queryFormAttributes, selectedSessionRecord) {
        var queryId = queryFormAttributes.queryId;
        return [{
            elementId: cowl.QE_SESSION_ANALYZER_VIEW_ID + "-" + queryId + "-" + selectedSessionRecord.cgrid,
            ...
            viewConfig: {
                ...
                selectedSessionRecord: selectedSessionRecord
            }
        }];
    }
