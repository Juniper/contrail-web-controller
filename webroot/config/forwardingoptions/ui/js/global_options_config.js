        ...
        columnHeader : {
            ...
            },
            {
                ...
                formatter: function(r, c, v, cd, dc) {
                    if(dc.property === 'Encapsulation Priority Order' && dc.value.length === 3) {
                            return '<span>' + dc.value[0] + '</span><br>' +
                                   '<span>' + dc.value[1] + '</span><br>' +
                                   '<span>' + dc.value[2] + '</span>';
...
function isPriorityChanged(oldPriority, newPriority) {
    var isChanged = true;
    if(oldPriority.length === 3 && newPriority.length === 3) {
        if(oldPriority[0] === newPriority[0] && oldPriority[1] === newPriority[1]
            && oldPriority[2] === newPriority[2]) {
            isChanged = false;
        }
    }
    return isChanged;
...
function populateData(result) {
    ...
    //prepare grid data
    gridDS.push({'property' : 'VxLAN Identifier Mode', 'value' : $("#ddVxLan").data("contrailDropdown").text()});
    if(priorities != null) {
        actPriorities = priorities;
        gridDS.push({'property' : 'Encapsulation Priority Order', 
            'value' : [encapsulationMap[priorities[0]],encapsulationMap[priorities[1]],encapsulationMap[priorities[2]]]});
    } else {
        actPriorities = ["MPLSoGRE", "MPLSoUDP", "VXLAN"];
        gridDS.push({'property' : 'Encapsulation Priority Order', 'value' : ["MPLS Over GRE","MPLS Over UDP","VxLAN"]});
    }
