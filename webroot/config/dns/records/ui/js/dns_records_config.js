...
dnsConfigObj = new DnsRecordsConfigObj();

function DnsRecordsConfigObj() {
...
function initActions() {
    ...
    btnCreateDNSOK.click(function (a) {
        ...
        if(null !== gvrConfig && typeof gvrConfig !== "undefined") { 
        	if(null === gvrConfig["dns_record"] || 
        		typeof gvrConfig["dns_record"] === "undefined" ||
        		null === gvrConfig["dns_record"]["dns_record_entry"] ||
        		typeof gvrConfig["dns_record"]["dns_record_entry"] === "undefined") {
        		gvrConfig["dns_record"] = {};
        		gvrConfig["dns_record"]["dns_record_entry"] = [];
        	}
    	    var dnsConfigData = gvrConfig["dns_records"]["dns_records_entry"];
    	    if($("#ddLocalServiceName").data("contrailCombobox").isEnabled() === false) {
    	    	...
    	    	for(var i=0; i<dnsConfigData.length; i++) {
	        		...
	        		if(dnsEntry["dns_records_name"] === 
	        			$(ddLocalServiceName).data("contrailCombobox").text().trim()) {
	                    dnsEntry["dns_records_name"] = $(ddLocalServiceName).data("contrailCombobox").text().trim();
	                    dnsEntry["dns_records_ip"] = $(txtLocalServiceIP).val().trim();
	                    dnsEntry["dns_records_port"] = parseInt($(txtLocalServicePort).val().trim());
            ...
            gvrConfig["global-vrouter-config"]["dns_records"] = {};
            gvrConfig["global-vrouter-config"]["dns_records"]["dns_records_entry"] = [];
        	gvrConfig["global-vrouter-config"]["dns_records"]["dns_records_entry"][0] = {};
        	var dnsEntry = gvrConfig["global-vrouter-config"]["dns_records"]["dns_records_entry"][0];
            dnsEntry["dns_records_name"] = $(ddLocalServiceName).data("contrailCombobox").text().trim();
            dnsEntry["dns_records_ip"] = $(txtLocalServiceIP).val().trim();
            dnsEntry["dns_records_port"] = parseInt($(txtLocalServicePort).val().trim());
            dnsEntry["ip_fabric_service_port"] = parseInt($(txtFabricServicePort).val().trim());
...
function populateData(result) {
    ...
    if(null !== result) {
        ...
        if(null !== gvrConfig["dns_records"] && 
            typeof gvrConfig["dns_records"] !== "undefined" &&
            null !== gvrConfig["dns_records"]["dns_records_entry"] &&
            typeof gvrConfig["dns_records"]["dns_records_entry"] !== "undefined" &&
            gvrConfig["dns_records"]["dns_records_entry"].length > 0) {
            var gridData = [];
            var dnsEntries = gvrConfig["dns_records"]["dns_records_entry"];
            for(var i=0; i<dnsEntries.length; i++) {
                var dnsEntry = dnsEntries[i];
                
                var localServiceName = dnsEntry["dns_records_name"];
                if(null === localServiceName || typeof localServiceName === "undefined")
                	localServiceName = "";

                var localAddress = "";
                var localServiceIP = dnsEntry["dns_records_ip"];
                ...
                var localServicePort = dnsEntry["dns_records_port"];
...
function showDnsEditWindow(mode, rowIndex) {
    if (mode === "add") {
        windowCreateDNS.find('h6.modal-header-title').text('Create Link Local Service');
...
function deleteDns(selected_rows) {
	if(null !== selected_rows && typeof selected_rows !== "undefined") {
		...
		if(null !== gvrConfigData && typeof gvrConfigData !== "undefined") {
			if(null !== gvrConfigData["dns_records"] &&
				typeof gvrConfigData["dns_records"] !== "undefined" &&
				null !== gvrConfigData["dns_records"]["dns_records_entry"] &&
				typeof gvrConfigData["dns_records"]["dns_records_entry"] !== "undefined" &&
				gvrConfigData["dns_records"]["dns_records_entry"].length > 0) {
				if($("#gridDNS").find("input.headerRowCheckbox")[0].checked === true) {
					delete gvrConfigData["dns_records"];
				} else {
					for(var i=0; i<selected_rows.length; i++) {
						var localServiceName = selected_rows[i].LocalServiceName.trim();
						var dnsEntries =
							gvrConfigData["dns_records"]["dns_records_entry"]; 
						for(var j=0; j<dnsEntries.length; j++) {
							if(localServiceName === 
								dnsEntries[j]["dns_records_name"].trim()) {
								(gvrConfigData["dns_records"]["dns_records_entry"]).splice(j,1);
							}
						}
					}
				}
