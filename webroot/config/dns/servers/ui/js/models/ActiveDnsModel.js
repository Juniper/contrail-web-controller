...
define([
    ...
    var ActiveDnsModel = ContrailModel.extend({
        ...
        addEditActiveDns: function(mode, callbackObj,
            ajaxMethod) {
            var ajaxConfig = {},
                returnFlag = false;
            var postData = {
            ...
            if (self.model().isValid(true,
                ...
                if (newactiveDnsData['display_name'] == '') {
                    newactiveDnsData['display_name'] =
                        newactiveDnsData['name'];
                }
                if (newactiveDnsData['fq_name'] == null ||
                    newactiveDnsData['fq_name'].length == 0
                ) {
                    newactiveDnsData['fq_name'] = [];
                    newactiveDnsData['fq_name'][0] = domain;
                    newactiveDnsData['fq_name'][1] =
                        newactiveDnsData['display_name'];
                }
                var nwIpams = newactiveDnsData[
                    'network_ipam_back_refs'];
                newactiveDnsData['virtual_DNS_data'][
                    'default_ttl_seconds'
                ] = parseInt(newactiveDnsData[
                    'virtual_DNS_data'][
                    'default_ttl_seconds'
                ]);
                ...
                newactiveDnsData['network_ipam_back_refs'] =
                    nwIpamBackRefs;
                delete newactiveDnsData['elementConfigMap'];

                console.log(newactiveDnsData[
                    'network_ipam_back_refs']);
