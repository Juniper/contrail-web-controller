...
define([
    ...
    var rowActionConfig = [
        ...
        ctwgc.getDeleteConfig('Delete', function(rowIndex) {
            ...
            DnsServerEditView.renderDeleteDnsServer({
                                  "title": ctwl.TITLE_DEL_DNS_SERVER + '('+ dataItem['display_name'] + ')', checkedRows: checkedRows, callback: function() {
                var dataView =
                    $(gridElId).data("contrailGrid")._dataView;
                dataView.refreshData();
            }});
        }),
	    ctwgc.getActiveDnsConfig('Active DNS Database', function(rowIndex) {
			    var dataItem =
                $(gridElId).data('contrailGrid')._dataView.getItem(rowIndex);
			    var name = dataItem.name, hashParams = null,
                ...
                options: {
                    ...
                    checkboxSelectable: {
                        onNothingChecked: function(e){
                            $('#btnActionDelDNS').addClass('disabled-link');
                        },
                        onSomethingChecked: function(e){
                            $('#btnActionDelDNS').removeClass('disabled-link');
                        }
                    },
                },
                dataSource: {
    ...
    DnsServerColumns = [
                {
                        ...
                        onClick : function(e, dc) {
							...
							layoutHandler.setURLHashParams({uuid : dc.uuid} ,{p : 'config_dns_records' ,merge : false ,triggerHashChange : true});
                ...
                },
        {
			...
			formatter: function(row, col, val, d, rowData) {
				var domain = getValueByJsonPath(rowData, 'virtual_DNS_data;domain_name', "-");
				return domain;
			}
                },
                    {
                        name: 'Forwarders',
			formatter: function(row, col, val, d, rowData) {
				var fwer = getValueByJsonPath(rowData, 'virtual_DNS_data;next_virtual_DNS', "-");
				return fwer;
                	}
		}
	...
	this.DnsDomIpamsFormatter = function(val, obj) {
		var nwIpams = getValueByJsonPath(obj, 'network_ipam_back_refs', null);
		if (null == nwIpams) {
			return "-";
		}
		var dispStr = "";
		...
		for (var i = 0; i < cnt; i++) {
			if (domain == nwIpams[i]['to'][0]) {
				dispStr += nwIpams[i]['to'][1] + ":" + nwIpams[i]['to'][2];
				if (i < cnt - 1) {
					dispStr += ", ";
				}
			}
	...
	this.DnsTtlFormatter = function(val, obj) {
		console.log('DnsTtlFormatter'+val);
		return val+"(seconds)";
	}
