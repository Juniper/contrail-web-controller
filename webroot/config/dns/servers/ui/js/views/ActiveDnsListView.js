...
define(
        ...
        function(_, ContrailView, ContrailListModel, ActiveDnsModel) {
                        ...
                        var listModelConfig = {
                                remote : {
                                    ajaxConfig : {
                                        ...
                                        url : '/api/tenants/config/sandesh/virtual-DNS/' + getCookie('domain') +
										       ":" + currentHashParams.focusedElement.dnsServer,
                                        type : "GET"
                                    },
                                    dataParser : function(response) {
                                        ...
                                        return activeDnsDataParser(response)
                                    }
                            ...
                            var contrailListModel = new ContrailListModel(
                                    listModelConfig);
                            pushBreadcrumb([currentHashParams.focusedElement.dnsServer]);
			...
			{
            if(response && response.length > i && response[i] && response[i].VirtualDnsRecordsResponse && response[i].VirtualDnsRecordsResponse.records && response[0].VirtualDnsRecordsResponse.records.list){
            var nextRecSetKey = response[i].VirtualDnsRecordsResponse.getnext_record_set;
            var res = response[i].VirtualDnsRecordsResponse.records.list.VirtualDnsRecordTraceData;
            if(res){
                if(res.length > i){
