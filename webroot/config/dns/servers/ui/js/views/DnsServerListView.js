...
define(
                        ...
                        var listModelConfig = {
                                remote : {
                                    ajaxConfig : {
                                        ...
                                        url : '/api/admin/config/get-data?type=virtual-DNS&count=4&fqnUUID=' + domainUUID,
                                        type : "GET"
                                    },
                                    dataParser : function(response) {
                                      ...
                                      /*  var gridElId = "#DnsServerGrid";
                                        ...
                                        return dnsServerDataParser(response)
                                    }
		...
		var vdnsData = getValueByJsonPath(result, "data;virtual_DNSs", []);
		var vdnsCnt = vdnsData.length;
                for (var i = 0; i < vdnsCnt; i++) {
			if ((null == vdnsData[i]) || (null == vdnsData[i]['virtual-DNS'])) {
			    continue;
			}
