...
define([
    'underscore',
    'contrail-view',
    'config/gohanUi/ui/js/models/svcInstModel',
    'config/gohanUi/ui/js/models/locationModel',
    'config/gohanUi/ui/js/views/instances/svcInstAddView',
    'config/gohanUi/ui/js/views/instances/svcInstEditView'
], function (_, ContrailView, SvcInstModel, LocationModel, SvcInstAddView, SvcInstEditView) {
    var svcInstAddView = new SvcInstAddView(),
                    ...
                    detail: {
                        template:
                            ...
                            onExpand: function(event, obj){
                                ctwu.getLocationGrid(event, obj, 'service_instances', ctwl.SERVICE_INSTANCES_GRID_ID);
                            }
                    },
    ...
    function rowActionConfig() {
        return [
            ...
            ctwgc.getEditConfig('Edit Local Service Instance', function(rowIndex) {
                var dataView = $(gridElId).data("contrailGrid")._dataView;
                var dataItem = $(gridElId).data('contrailGrid')._dataView.getItem(rowIndex);
                var ajaxConfig = {
                        url: './gohan_contrail/v1.0/tenant/service_instances/'+dataItem.id+'/local_service_instance?sort_key=id&sort_order=asc&limit=25&offset=0&tenant_id='+dataItem.tenant_id,
                        type:'GET'
                    };
                contrail.ajaxHandler(ajaxConfig, null, function(model){
                    ...
                    svcInstEditView.renderLocationGridPopup({
                                         "title": 'Update Local Service Instance',
                                          callback: function () {
                                              dataView.refreshData();
                                          }});
               },function(error){
        ...
        var headerActionConfig = [
            {
                ...
                "onClick": function() {
                    ...
                    svcInstAddView.model = svcInstModel;
                    svcInstModel.editView = svcInstAddView;
                    svcInstAddView.renderConfigureSvcInst({
