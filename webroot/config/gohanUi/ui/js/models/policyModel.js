...
define([
    ...
    'config/gohanUi/ui/js/models/ruleModel'
    ...
    var policyModel = ContrailConfigModel.extend({
        ...
        showHideServiceInstance: function(ruleModels) {
            ruleModels.showService = ko.computed((function() {
                if (this.apply_service_check() == true) {
                 ...
                 var error = $('#PolicyRules > table > tbody > tr:last-child > .error-cell > span').text();
                 if(error === 'Select atleast one service to apply.'){
        ...
        configurePolicy: function (mode, callbackObj) {
            ...
            var newPolicyData = $.extend(true,{},this.model().attributes),
                ...
                if(mode === 'add'){
                    type = "POST";
                    url = "./gohan_contrail/v1.0/tenant/network_policies";
                }else{
                    type = "PUT";
                    url = "./gohan_contrail/v1.0/tenant/network_policies/"+newPolicyData.id;
                }
        ...
        deletePolicy: function(selectedGridData, callbackObj) {
            ...
            var type = "DELETE";
            var url = "./gohan_contrail/v1.0/tenant/network_policies/"+selectedGridData.id;
            var postData = {};
            postData["network_policy"] = {};
            ajaxConfig.type = type;
            ajaxConfig.data = JSON.stringify(postData);
