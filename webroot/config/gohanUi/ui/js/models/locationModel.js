    ...
    var LocationModel = ContrailConfigModel.extend({
        defaultConfig: {
            ...
            deleteRules: function(data, rules) {
                ...
                if(model.locationId() != ''){
                    locationStack.push({locationId : model.locationId(), svcTempId :model.svcTempId()});
                }
            ...
            configureLocation: function (parentName, callbackObj) {
                ...
                if(addedLocationVal.length === 0){
                    ...
                    if(emptyLocationId && locationStack.length === 0){
                        var error = {};
                        error.responseText = 'Please Add the row or Delete the row.';
                        if (contrail.checkIfFunction(callbackObj.error)) {
                            callbackObj.error(error);
                        }
                        returnFlag = false; 
                          ...
                          for(var l = 0; l < addedLocationVal.length; l++){
                                if(addedLocationVal[l].locationId() === ''){
                                    locationStack.push({locationId : addedLocationVal[l].locationId(), locationName : addedLocationVal[l].locationName().split(';')[0],
                                     name: addedLocationVal[l].name(), description: addedLocationVal[l].description()});
                                }
                          }
