...
function setLogicalRouterRead(error, logicalRouterConfig, networkUUID, addVMIData, resetExternalGateway, request, response, appData)
{
    ...
    if(addVMIData != null && addVMIData.length > 0){
        ...
        updateRouteTable(addVMIData, fqNameDomain, fqNameproject, lruuid, appData, function (error, data){
            ...
            updteExternalNetwork(logicalRouterConfig, networkUUID, resetExternalGateway, request, response, appData);
        });
    } else {
        updteExternalNetwork(logicalRouterConfig, networkUUID, resetExternalGateway, request, response, appData);
    }
...
function updteExternalNetwork(logicalRouterConfig, networkUUID, resetExternalGateway, request, response, appData)
{
	var routerObj = {};
    routerObj["router"] = {};
    ...
    if (config.network.router_L3Enable === true){
        if (resetExternalGateway == true){
            updateExternalNetwork(request, routerObj, routerUUID, function (error, data){
                if (error){
...
function updateExternalNetwork (request, routerObj, routerUUID, callback)
{
