...
define([
        ...
        render: function () {
            ...
            var postData = {
                "data": [{
                    ...
                    "fields": ["interface_route_table_routes"],
    ...
    function rtTableParser (response) {
        if ((null == response) || (null == response[0]) ||
            (null == response[0]['interface-route-tables'])) {
            return [];
        }
        var results = [];
        var rtTabs = response[0]['interface-route-tables'];
        var cnt = rtTabs.length;
        for (var i = 0; i < cnt; i++) {
            var rts =
                getValueByJsonPath(rtTabs[i],
                                   'interface-route-table;interface_route_table_routes;route',
                                   null);
            if (null != rts) {
                rtTabs[i]['interface-route-table']['route'] = rts;
                delete rtTabs[i]['interface-route-table']['routes'];
            } else {
                rtTabs[i]['interface-route-table']['route'] = [];
            }
            results.push(rtTabs[i]['interface-route-table']);
        }
        return results;
    };
