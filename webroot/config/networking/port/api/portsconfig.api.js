...
function deleteAllPorts (req, res, appData)
{
    ...
    configApiServer.apiGet(reqUrl, appData, function(err, vmiData) {
        ...
        var parentID = []
        for (var i = 0; i < vmiCnt; i++) {
            if (!("virtual_machine_interface_refs" in vmiData['virtual-machine-interfaces'])) {
                //Normal port
                console.log("Normal"+vmiData['virtual-machine-interfaces'][i]['uuid']);
                dataObjArr[0]['deleteIDs'].push(vmiData['virtual-machine-interfaces'][i]['uuid']);
            } else {
                if ("virtual_machine_interface_refs" in vmiData['virtual-machine-interfaces'] && 
                    "virtual_machine_interface_properties" in vmiData['virtual-machine-interfaces'] && 
                    "sub_interface_vlan_tag" in vmiData['virtual-machine-interfaces']["virtual_machine_interface_properties"] && 
                    vmiData['virtual-machine-interfaces']["virtual_machine_interface_properties"]["sub_interface_vlan_tag"] != null)
                    {
                        //It must be a subinterface
                        console.log("subinterface"+vmiData['virtual-machine-interfaces'][i]['uuid']);
                        dataObjArr[0]['deleteIDs'].push(vmiData['virtual-machine-interfaces'][i]['uuid']);
                } else {
                    //It is Primary Interface
                    console.log("subinterface"+vmiData['virtual-machine-interfaces'][i]['uuid']);
                    parentID.push(vmiData['virtual-machine-interfaces'][i]['uuid']);
                }
            }
        }
        var parentIDLen = parentID.length;
        for (var i = 0; i < parentIDLen; i++) {
            dataObjArr[0]['deleteIDs'].push(parentID[i]);
        }
