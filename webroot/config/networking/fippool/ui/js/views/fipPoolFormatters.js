...
define([
    ...
    var FipPoolFormatters = function() {
        ...
        self.projectName = function(d, c, v, cd, dc) {
            var tenant = '';
            if(dc['perms2']['share']){
                for(var i = 0; i < dc['perms2']['share'].length; i++){
                    if(dc['perms2']['share'][i]['tenant_name'] != undefined){
                        tenant += dc['perms2']['share'][i]['tenant_name'] + "<br />"
                    }
                }
                var tenantFormat = tenant.split("<br />");
                if(tenantFormat.length > 3){
                    var tenantFormatMore = tenantFormat.length-3
        ...
        self.vnDDFormatter = function(response, edit, fipModel) {
            if(!edit && response.length > 0) {
                fipModel.model.virtualNetworkName(response[0].value);
            }
            return response;
        }
