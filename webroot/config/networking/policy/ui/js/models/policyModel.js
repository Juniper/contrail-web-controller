        ...
        formatModelConfig: function (config) {
            ...
            if(rulesList != null && rulesList.length > 0) {
                for(var i = 0; i < rulesList.length; i++) {
                    if("src_addresses" in rulesList[i] &&
                      rulesList[i]["src_addresses"][0]["subnet"] != null){
                        var subnet = rulesList[i]["src_addresses"][0]["subnet"];
                        var newText = subnet["ip_prefix"]
                             + "/" + subnet["ip_prefix_len"];
                        rulesList[i]["src_customValue"] = {
                                                           "text": newText,
                                                           "groupName": "CIDR"
                                                          };
                    }
                    if("dst_addresses" in rulesList[i] && 
                       rulesList[i]["dst_addresses"][0]["subnet"] != null){
                        var subnet = rulesList[i]["dst_addresses"][0]["subnet"];
                        var newText = subnet["ip_prefix"]
                             + "/" + subnet["ip_prefix_len"];
                        rulesList[i]["dst_customValue"] = {
                                                           "text": newText,
                                                           "groupName": "CIDR"
                                                          };
                    }
            ...
            if (this.model().isValid(true, "policyValidations")) {
                ...
                for (var i=0;i<policeyRuleLen;i++){
                    ...
                    newPoliceyRule[i].action_list.simple_action =
                      (newPoliceyRule[i].action_list.simple_action).toLowerCase();
                    ...
                    if(policeyRule[i].service_instance() == "") {
                        ...
                        newPoliceyRule[i].action_list.apply_service[0] =
                            policeyRule[i].service_instance();
                    }
