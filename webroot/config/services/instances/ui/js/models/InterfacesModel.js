...
define([
    ...
    var InterfacesModel = ContrailModel.extend({
        ...
        addStaticRt: function(data, obj, parentM, text) {
            var intfType = this.model().attributes.interfaceType();
            if (null == intfType) {
                return;
            }
            intfType = intfType.toLowerCase();
            var intfTypes = window.intfTypes;
            if (null == intfTypes) {
                return;
            }
            var idx = intfTypes.indexOf(intfType);
            if (-1 == idx) {
                return;
            }
