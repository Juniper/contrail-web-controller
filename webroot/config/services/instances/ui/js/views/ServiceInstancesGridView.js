...
define([
    ...
    'config/services/instances/ui/js/views/ServiceInstancesEditView'
], function (_, ContrailView, ServiceInstancesModel, ServiceInstancesEditView) {
    var serviceInstancesEditView = new ServiceInstancesEditView(),
    gridElId = "#" + ctwl.SERVICE_INSTANCES_GRID_ID;

    var ServiceInstancesGridView = ContrailView.extend({
        el: $(contentContainer),
        render: function () {
            var self = this,
            ...
            self.renderView4Config(self.$el, self.model,
                                   getServiceInstancesGridViewConfig(pagerOptions));
        }
    });

    var getServiceInstancesGridViewConfig = function (pagerOptions) {
        return {
    ...
    var rowActionConfig = [
        ctwgc.getEditConfig('Edit', function(rowIndex) {
            ...
            var serviceInstancesModel = new ServiceInstancesModel(dataItem);
            showHideModelAttr(serviceInstancesModel);
            serviceInstancesEditView.model = serviceInstancesModel;
            serviceInstancesEditView.renderConfigureServiceInstances({
        ...
        ctwgc.getDeleteConfig('Delete', function(rowIndex) {
            var serviceInstancesModel = new ServiceInstancesModel();
            var dataItem =
                $(gridElId).data('contrailGrid')._dataView.getItem(rowIndex);

            var checkedRows = [dataItem];
            serviceInstancesEditView.model = serviceInstancesModel;
            serviceInstancesEditView.renderDeleteServiceInstances({
    ...
    function getHeaderActionConfig() {
        var headerActionConfig = [
            {
                "type": "link",
                "title": ctwl.TITLE_ADD_SERVICE_INSTANCE,
                "iconClass": "icon-plus",
                "onClick": function() {
                    window.intfToBeTaken = null;
                    window.intfTypes = [];
                    serviceInstancesModel = new ServiceInstancesModel();
                    showHideModelAttr(serviceInstancesModel);
                    serviceInstancesEditView.model = serviceInstancesModel;
                    serviceInstancesEditView.renderConfigureServiceInstances({
                        "title": ctwl.TITLE_EDIT_SERVICE_INSTANCES,
                        "isEdit": false,
                        callback: function() {
                            var dataView =
                                $(gridElId).data("contrailGrid")._dataView;
   ...
   return ServiceInstancesGridView;
});
