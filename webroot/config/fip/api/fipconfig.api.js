...
function getFipsForProjectCb (results, response, appData)
{
    ...
    fipListAggCb(null, results, function (err, fipAggList) {
        ...
        for (var i = 0; i < fipBackRefsCnt; i++) {
            ...
            if ('floating-ip' in fipBackRef &&
                ...
                for (var j = 0; j < vmiRefsCnt; j++) {
                    ...
                    if (null == vmiObjs[vmiRef['uuid']]) {
                        vimObjArr.push(vmiRef['uuid']);
                        vmiObjs[vmiRef['uuid']] = vmiRef['uuid'];
                    }
                }
            }
        }
        ...
        var chunk = 200;
        var fipObjArrLen = vimObjArr.length;
        for (var i = 0, j = fipObjArrLen; i < j; i += chunk) {
            var tmpArray = vimObjArr.slice(i, i + chunk);
        ...
        async.map(dataObjGetArr,
            ...
            for (var i = 0; i < vmiCnt; i++) {
                tmpVMIObjs[vmiData[i]['virtual-machine-interface']['uuid']] =
                    vmiData[i]['virtual-machine-interface']['virtual_machine_refs'];
            }
            var fipData = fipAggList['floating_ip_back_refs'];
            var fipDataCnt = fipData.length;
            for (var i = 0; i < fipDataCnt; i++) {
                fipAggList['floating_ip_back_refs'][i] =
                    getInstanceIPForVirtualMachineInterface(tmpVMIObjs, fipData[i]);
            }
            commonUtils.handleJSONResponse(null, response, fipAggList);
        });
    });
 ...
 */
function getInstanceIPForVirtualMachineInterface (vmiObjs, fip)
{
    var vmiRefsCnt =
