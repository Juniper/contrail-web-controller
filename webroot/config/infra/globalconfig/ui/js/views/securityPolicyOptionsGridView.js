...
define([
...
], function (_, ContrailView, SecPolicyOptionsModel, SecPolicyOptionsEditView, GlobalConfigFormatters) {
    var secPolicyOptionsEditView = new SecPolicyOptionsEditView(),
        globalConfigFormatters = new GlobalConfigFormatters(),
        gridElId = "#" + ctwc.GLOBAL_POLICY_MGMT_OPTIONS_GRID_ID;

    var secPolicyOptionsGridView = ContrailView.extend({
        el: $(contentContainer),
        render: function () {
            var self = this,
    ...
    var getSecPolicyOptionsGridViewConfig = function (viewConfig) {
        return {
            elementId: cowu.formatElementId([ctwc.GLOBAL_POLICY_MGMT_OPTIONS_LIST_VIEW_ID]),
            view: "SectionView",
            viewConfig: {
                rows: [
                    {
        ...
        var headerActionConfig = [
            {
                ...
                "onClick": function() {
                    ...
                    if(isGlobal) {
                        ...
                        ajaxConfig.data = JSON.stringify({data:
                            [{type: 'projects', obj_uuids: viewConfig.projectId}]});
                    }
                    contrail.ajaxHandler(ajaxConfig, null, function(result) {
                        var secPolicyMgmtConfig;
                        if(isGlobal) {
                            var globalSecOptions = _.get(result, '0.policy-managements', []);
                            secPolicyMgmtConfig = _.find(globalSecOptions, function(policyMgmt) {
                                                      var fqName = _.get(policyMgmt, 'policy-management.fq_name.0', '');
                                                      return fqName === ctwc.DEFAULT_POLICY_MANAGEMENT;
                                                   });
                            secPolicyMgmtConfig = _.get(secPolicyMgmtConfig, 'policy-management', {});
                        } else {
                            secPolicyMgmtConfig = _.get(result, "0.projects.0.project", {});
                        }
                        secPolicyOptionsModel = new SecPolicyOptionsModel(secPolicyMgmtConfig);
                        secPolicyOptionsEditView.model = secPolicyOptionsModel;
                        secPolicyOptionsEditView.renderEditSecPolicyOptions({
                                      "title": ctwl.TITLE_EDIT_SECURITY_POLICY_OPTIONS,
                                      isGlobal: isGlobal,
                                      callback: function() {
                            var dataView =
