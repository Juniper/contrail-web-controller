...
define([
    ...
    var securityPolicyOptionsListView = ContrailView.extend({
        el: $(contentContainer),
        render: function () {
            var self = this,
            ...
            this.renderView4Config(this.$el,
                    contrailListModel, getSecPolicyOptionsGridViewConfig());
        },
        parseSecPolicyOptionsData : function(result){
            var gridDS = [],
                result = _.get(result, '0.policy-managements', []),
                ...
                globalPolicyMgmt = _.get(secPolicyMgmtConfig, 'policy-management', {});
            _.each(ctwc.GLOBAL_POLICY_MANAGEMENT_OPTIONS_MAP, function(polMgmtOption){
                gridDS.push({name: polMgmtOption.name, key: polMgmtOption.key,
                    value: globalPolicyMgmt[polMgmtOption.key] ? globalPolicyMgmt[polMgmtOption.key] : false});
            });
            return gridDS;
        }
    });
