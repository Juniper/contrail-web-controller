    ...
    var vRouterCfgModel = ContrailConfigModel.extend({
        ...
        addSubnetByIndex: function(data, kbSubnet) {
          ...
          setSubnetAllocationPool: function (self, model, text) {
              var subnetsItem = self.model().attributes['network_ipam_refs'].toJSON();
              var subnetIndex = subnetsItem.length - 1;
              var subnet = subnetsItem[subnetIndex];
              var cid = model.cid;
                  ...
                  if(ipPrefixLen <= 32){
                      ...
                      var index = self.network_ipam_refs().length - 1;
                      self.network_ipam_refs()[index].disable(true);
                  }
                  else{
