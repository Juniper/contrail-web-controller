    ...
    btnCreateRTOK.click(function (a) {
        ...
        if (routeTuples && routeTuples.length > 0) {
            for (var j = 0; j < routeTuples.length; j++) {
                var divid = routeTuples[j].id;
                var id = getID(divid);
                var currentRouteText = $("#routeTuples_"+id+"_txtRoutePrefix").val().trim();
                var currentNextHopText = null;
                if($("#routeTuples_"+id+"_txtNextHop").val().trim() !== "")
                    currentNextHopText = $("#routeTuples_"+id+"_txtNextHop").val().trim();
