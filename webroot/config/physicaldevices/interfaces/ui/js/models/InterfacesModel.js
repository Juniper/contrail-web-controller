...
define([
    ...
    var InterfacesModel = ContrailModel.extend({
        ...
        formatModelConfig: function (modelConfig) {
             ...
             */
            var servers = (modelConfig['serverDetails'] != null) ?
                (modelConfig['serverDetails']) : [],
                serverModels = [],
            ...
            if(servers.length > 0) {
                for(var i = 0; i < servers.length; i++) {
                    var formattedServer = servers[i];
                    var mac = formattedServer.mac;
                    var ip = formattedServer.ip;
                    if(ip != null && ip != '-' && ip != ''){
                        mac = mac + ' (' + ip + ')';
                    } else {
                        ip = ' ';
                    ...
                    serverModel = new ServersModel(
                        {
                            ...
                            isVMICreate : false,
                            disable : true,
                            uuid : formattedServer.uuid,
                        }
                    );
                    serverModel.__kb.view_model.model().on('change:mac',
                        function(model, text){
        ...
        addServer: function() {
            ...
            newServer.__kb.view_model.model().on('change:mac',
                function(model, text){
                     self.setServerIP(model, text);
                }
            );
        ...
        setServerIP : function(item, text) {
            ...
            if(model != null) {
                ...
                if(ipObj.length === 2) {
                   model.ip(ipObj[1].replace(')','').replace('(',''));
        ...
        createPort : function(portDetails, attr, callbackObj, ajaxOpt,
            ...
            var postObjInput = {};
            var ajaxConfig = {};
            var selVN = self.getSelectedVNItem(attr.virtual_network);
            ...
            contrail.ajaxHandler(ajaxConfig, function () {
                ...
                if(result != null && result['virtual-machine-interface']
                    ...
                    if (attr.logical_interface_type === ctwl.LOGICAL_INF_L3_TYPE
                        && attr.subnet.trim() != '') {
                        self.setVMIRefsToSubnet(vmiId, attr.subnet.trim());
                    }
                } else {
                    callbackObj.success();
                }
        ...
        createUpdatePhysicalInterface : function(attr, callbackObj, ajaxOpt,
            ...
            var vlan =  attr.logical_interface_vlan_tag === '' ?
                0 : parseInt(attr.logical_interface_vlan_tag);
        ...
        validations: {
            configureValidation: {
                ...
                subnet : function(value, attr, finalObj){
                    if(finalObj.logical_interface_type ===
                        ctwl.LOGICAL_INF_L3_TYPE) {
                        if(finalObj.virtual_network != 'none'
                            && value.split("/").length != 2) {
                            return "Enter a valid Subnet in xxx.xxx.xxx.xxx/xx";
                        }
                    }
                },
                virtual_network : function(value, attr, finalObj){
                    if(finalObj.logical_interface_type ===
                        ctwl.LOGICAL_INF_L3_TYPE) {
                        if(finalObj.virtual_network === 'none') {
                            return "Please select a Virtual Network";
                        }
                   }
                }
            }
