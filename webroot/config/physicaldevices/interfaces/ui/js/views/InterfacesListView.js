            ...
            if(result!= null && result.length > 0) {
                for(var i = 0; i < result.length; i++) {
                    ...
                    gridDS.push({
                        ...
                        parent : parentName,
                        logical_interface_vlan_tag : liDetails.vlanTag != null ?
                            liDetails.vlanTag : '-',
                        server : (liDetails.liType == "L3")? '-' :
                            vmiDetails != null ? vmiDetails : '-',
                        servers_display : (liDetails.liType == "L3")? '-' :
                            serverString,
                        ...
                        subnetCIDRArr : liDetails.subnetCIDRArr.length > 0 ?
                            liDetails.subnetCIDRArr : '-',
                        virtual_network :
                            liDetails.vnUUID != null ?
                            liDetails.vnUUID : 'none',
                        serverDetails : liDetails.serverDetails
                    });
                }
            }
            return gridDS;
                ...
                for(var i = newData.length - 1; i > -1; i--) {
                    if(result[j]['logical-interface'].parent_uuid ===
                        ...
                        var infObj = {
                            ...
                            parent : parentName,
                            logical_interface_vlan_tag :
                                liDetails.vlanTag != null ?
                                liDetails.vlanTag : '-',
                            server : (liDetails.liType ==
                                ctwl.LOGICAL_INF_L3_TYPE)? '-' :
                                vmiDetails != null ? vmiDetails : '-',
                            servers_display : serverString,
                            vn : liDetails.vnRefs != null ?
                                liDetails.vnRefs : '-',
                            logical_interface_type : liDetails.liType != null ?
                                liDetails.liType : '-' ,
                            ...
                            subnetCIDRArr : liDetails.subnetCIDRArr.length > 0 ?
                                liDetails.subnetCIDRArr : '-',
                            virtual_network : liDetails.vnUUID != null ?
                                liDetails.vnUUID : 'none',
                            serverDetails : liDetails.serverDetails
                        };
