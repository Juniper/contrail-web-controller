...
function serviceAppliancesConfigObj() {
  ...
  var gridDS = [];
    ...
    function initActions() {
        ...
        for(var i = 0; i < selectedPI.length; i++) {
          ...
          if(serviceApplianceSize % 2 === 0){
            sa["service-appliance"]["physical_interface_refs"][i]["attr"]["interface_type"] = "left";
          }else{
            sa["service-appliance"]["physical_interface_refs"][i]["attr"]["interface_type"] = "right";
          }
        }       
  ...
  window.failureHandlerForPhysicalInterfaces =  function(error) {
    ...
    if(gridDS.length > 0) {
	  serviceApplianceSize = gridDS.length;
