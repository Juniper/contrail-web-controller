...
function updatePhysicalInterface(request, response, appData) {
    var url = '/physical-interface/';
    var saId = null;

    if (piId = request.param('id').toString()) {
        url += piId;
    } else {
        error = new appErrors.RESTServerError('Physical Interface ID is required.');
        commonUtils.handleJSONResponse(error, response, null);
        return;
    }
    var postData = request.body;
...
exports.updatePhysicalInterface = updatePhysicalInterface;