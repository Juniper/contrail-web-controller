...
function getInterfaceDetails (req, res, appData)
{
    ...
    console.log("Sending LI:", new Date());
    async.map(dataObjGetArr,
              commonUtils.getServerResponseByRestApi(configApiServer, true),
              function(err, results) {
    console.log("Got LI:", new Date());
        ...
        if (!dataObjGetArr.length) {
            console.log("NOT FOUND, VMI and IP");
            commonUtils.handleJSONResponse(null, res, liData);
            return;
        }
        console.log("Caklling INST IP:", new Date());
        async.map(dataObjGetArr,
                  commonUtils.getServerResponseByRestApi(configApiServer, true),
                  function(err, results) {
            console.log("Done INST IP:", new Date());
            var vmiResp = [];
            var instIpsResp = [];
            var resCnt = results.length;
            for (var i = 0; i < resCnt; i++) {
            ...
            if (!dataObjGetArr.length) {
                console.log("NOT GETTING SUBNET");
                var newLiData = 
            ...
            console.log("SUBNET:", new Date());
            var subnetData = [];
            async.map(dataObjGetArr,
                      commonUtils.getServerResponseByRestApi(configApiServer, true),
                      function(err, results) {
                console.log("Got SUBNET:", new Date());
                var resCnt = results.length;
                for (var i = 0; i < resCnt; i++) {
                ...
                console.log("getting SUBNET alsp");
                commonUtils.handleJSONResponse(null, res, newLiData);
                return;
            });
        });
...
{
    var compVMIData = {};
    console.log("BUILDData stsrt :", new Date());
    var instIpsCnt = 0;
    if (null != instIpsResp) {
    ...
    for (i = 0; i < instIpsCnt; i++) {
        ...
        if (null !=
            ...
            console.log("Getting VM Refs as:",
                        vmiObjs[vmiId]['virtual-machine-interface']['virtual_machine_refs']);
            vmiDetails['vm_refs'] =
                vmiObjs[vmiId]['virtual-machine-interface']
                ['virtual_machine_refs'];
        }
    ...
    for (var key in vmiObjs) {
        ...
        console.log("We have not put yet key:", key);
    ...
    console.log("BUILT Done:", new Date());
