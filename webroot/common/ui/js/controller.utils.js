...
define([
    ...
    var CTUtils = function () {
        ...
        this.getSecurityRulesGrid = function(e, obj, tenantName, id){
            ...
            var ajaxConfig = {
                    url: './gohan_contrail/v1.0/tenant/'+tenantName+'/'+obj.id+'/security_group_rules?sort_key=id&sort_order=asc&limit=25&offset=0',
                    type:'GET',
                    async: false
                };
            contrail.ajaxHandler(ajaxConfig, null, function(model){
                ...
                table.appendChild(tbody);
                region.appendChild(table);
                self.getLocalSecurityGroup(region, content, obj.id);
            },function(error){
                contrail.showErrorMsg(error.responseText);
            });
        };
        this.getLocalSecurityGroup = function(region, content, id){
            var ajaxConfig = {
                    url: './gohan_contrail/v1.0/tenant/security_groups/'+id+'/local_security_groups?sort_key=id&sort_order=asc&limit=25&offset=0',
                    type:'GET',
                    async: false
                };
            contrail.ajaxHandler(ajaxConfig, null, function(model){
