...
define([
    ...
    var testSuiteClass = function (CTParser, suiteConfig){
        ...
        dataParserTestGroup.registerTest(cotr.test("Test valid ESI", function() {
            //Declare the number of assert statements in current test case
            equal(JSON.stringify(CTParser.isValidEsi(mockData.esiInput.ALL_00)), 'true' , 'Invalid ESI with all Zero');
            equal(JSON.stringify(CTParser.isValidEsi(mockData.esiInput.ALL_FF)), 'true' , 'Invalid ESI with all F');
            equal(JSON.stringify(CTParser.isValidEsi(mockData.esiInput.INVALID_ESI)), 'true' , 'Invalid ESI');
