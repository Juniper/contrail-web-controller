...
var tenantNetworkMonitorUtils = {
    ...
    formatVN: function(vn) {
        ...
        if($.isArray(vn)) {
            ...
            var fqNameArr = vn.split(':');
            if(fqNameArr.length == 3)
                formattedValue =  fqNameArr[2] + ' (' + fqNameArr[1] + ')';
            else
    ...
    parseInstDetails: function(data,rowData) {
        ...
        $.each(interfaces,function(idx,obj) {
            ...
            var ipAddr = "";
            if(obj['ip6_active'])
                ipAddr += wrapLabelValue('IPv4',ifNull(obj['ip_address'],'-'))+"<br/>"+wrapLabelValue('IPv6',ifNull(new v6.Address(obj['ip6_address']).correctForm(),'-'))
                +"<br/> "+wrapLabelValue('Mac',ifNull(obj['mac_address'],'-'));
            else
                ipAddr += wrapLabelValue('IPv4',ifNull(obj['ip_address'],'-'))+"<br/>"+wrapLabelValue('Mac',ifNull(obj['mac_address'],'-'));
            intfStr[idx] = [uuid, ipAddr, ifNull(obj['label'],'-'),ifNull(tenantNetworkMonitorUtils.formatVN(obj['virtual_network']),'-'),
