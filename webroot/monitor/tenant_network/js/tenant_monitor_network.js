...
function getConnectedTitleFromContextObj(ctxObj) {
    if(ctxObj['srcVN'] == ctxObj['fqName']) {
        return contrail.format("with in virtual network {0}",stripOffProject(ctxObj['srcVN']));
    } else
        return contrail.format("between virtual network {0} and {1}",stripOffProject(ctxObj['srcVN']),stripOffProject(ctxObj['fqName']));
}
