...
function getGlobalControllerOverview (req, res, appData)
{
    ...
    commonUtils.createReqObj(dataObjArr, reqUrl, null, null, opApiServer, null,
                             appData);
    reqUrl = "/analytics/uves/vrouter/*?flat&cfilt=UVEAlarms";
    commonUtils.createReqObj(dataObjArr, reqUrl, null, null, opApiServer, null,
                             appData);
    ...
    async.map(dataObjArr,
        ...
        var vrNodesAlarms = commonUtils.getValueByJsonPath(results, "1;value", []);
        var controlNodes = commonUtils.getValueByJsonPath(results, "2", []);
        var controlNodesAlarms = commonUtils.getValueByJsonPath(results, "3;value", []);
        var anaNodes = commonUtils.getValueByJsonPath(results, "4;value", []);
        var anaNodesDownAlarmsList = commonUtils.getValueByJsonPath(results, "5;value", []);
        var configNodes = commonUtils.getValueByJsonPath(results, "6", []);
        var configNodesDownAlarmList = commonUtils.getValueByJsonPath(results, "7", []);
        var databaseNodes = commonUtils.getValueByJsonPath(results, "8", []);
        var databaseNodesDownAlarmList = commonUtils.getValueByJsonPath(results, "9", []);
        ...
        resultJSON['vrNodesAlarms'] = vrNodesAlarms;
        resultJSON["controlNodesCnt"] = controlNodes.length;
        resultJSON["controlNodesAlarms"] = controlNodesAlarms;
        resultJSON["anaNodesCnt"] = anaNodes.length;
        resultJSON["anaNodesDownList"] = anaNodesDownAlarmsList;
        resultJSON["configNodesCnt"] = configNodes.length;
        resultJSON["configNodesDownAlarmList"] = configNodesDownAlarmList;
        resultJSON["databaseNodesCnt"] = databaseNodes.length;
        resultJSON["databaseNodesDownAlarmList"] = databaseNodesDownAlarmList;
