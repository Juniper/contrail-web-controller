                ...
                this.parseConfigNodesDashboardData = function (result) {
                    ...
                    $.each(result,function(idx,d) {
                        ...
                        for (var key in memCpuUsage) {
                            if (memCpuUsage.hasOwnProperty(key) && key.startsWith('contrail-api')) {
                                var memcpu = memCpuUsage[key];
                                cpu += parseInt(getValueByJsonPath(memcpu,'cpu_share'));
                                mem += parseInt(getValueByJsonPath(memcpu,'mem_res'))/1024;
                            }
                          }
                        obj['x'] = cpu;
                        obj['y'] = mem;
