    ...
    function getStatusesForAllDbProcesses(processStateList){
        ...
        if(processStateList != null){
           for(var i=0; i < processStateList.length; i++){
              ...
              if(currProc.process_name == monitorInfraConstants.UVEModuleIds['KAFKA']){
                 ret[monitorInfraConstants.UVEModuleIds['KAFKA']] = getProcessUpTime(currProc);
              } else if(currProc.process_name == monitorInfraConstants.UVEModuleIds['DATABASE']){
                 ret[monitorInfraConstants.UVEModuleIds['DATABASE']] = getProcessUpTime(currProc);
              }
           }
        }
        return ret;
