...
define([
    ...
    function getAnalyticsMessages(aNodeData) {
        var msgs = monitorInfraUtils.getAnalyticsMessagesCountAndSize(aNodeData,['contrail-collector']);
        ...
        if(processStateList != null){
            for(var i=0; i < processStateList.length; i++){
                ...
                if (currProc.process_name == "contrail-query-engine"){
                    ret[monitorInfraConstants.UVEModuleIds['QUERYENGINE']] = getProcessUpTime(currProc);
                }  else if (currProc.process_name == "contrail-analytics-nodemgr"){
                    ret[monitorInfraConstants.UVEModuleIds['ANALYTICS_NODEMGR']] = getProcessUpTime(currProc);
                }  else if (currProc.process_name == "contrail-analytics-api"){
                    ret[monitorInfraConstants.UVEModuleIds['APISERVER']] = getProcessUpTime(currProc);
                } else if (currProc.process_name == "contrail-collector"){
                    ret[monitorInfraConstants.UVEModuleIds['COLLECTOR']] = getProcessUpTime(currProc);
                }
            }
        }
        return ret;
